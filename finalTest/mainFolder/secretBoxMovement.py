...
# Scans the table based on joint location to find where to put the lid
...
def scanTable(rtde_c, rtde_r):
    ...
    tableHomeJ = [0.8427166938781738, -2.0175072155394496, -0.7320303916931152,
...
# This function moves to scan the secret
# box by defined joints to know the lid location
def lidLocation(rtde_c, rtde_r):
    ...
    # scanBoxPose = [0.44369501072561013, 0.09707740459758275,
    # 0.1554048423662069, np.deg2rad(roll), np.deg2rad(pitch), np.deg2rad(yaw)]

    scanBoxJoints = [0.600801944732666,
                     -1.433079556827881,
                     -2.5380635261535645,
                     3.9887096124836425,
                     -1.446258846913473,
...
# Picks up lid based on the boxLoc from the lidLocation function
# Then places it on the table to scan the secret ID
def pickUpLid(rtde_c, rtde_r, boxLoc,
              gripperSecretLid, tableFitLoc, gripperOpen):
    velocity = 1
    acceleration = 1
    blend = 0.0
    ...
    scanBoxPose = [0.44369501072561013,
                   0.09707740459758275,
                   0.15540484236620697,
    ...
    avoidBoxOne = [boxLoc[0], boxLoc[1], boxLoc[2],
                   np.radians(-30), 0, 0]
    ...
    avoidBoxOne = [boxLoc[0], boxLoc[1] - 0.01,
                   boxLoc[2], np.radians(-30), 0, 0]

    boxPosRef = rtde_c.poseTrans(scanBoxPose, avoidBoxOne)

    rtde_c.moveL(boxPosRef, velocity, acceleration, blend)

    avoidBoxOne = [0, boxLoc[1] - 0.01, -0.05,
                   np.radians(-30), 0, 0]
    ...
    avoidBoxOne = [0, boxLoc[1] + 0.09, -0.05,
                   np.radians(-30), 0, 0]
    ...
    scanTableBoxJoints = [0.8427166938781738,
                          -2.0175072155394496,
                          -0.7320303916931152,
                          1.1957790094562988,
                          -1.5637462774859827,
    ...
    # Lay the lid down
    tableFitLoc = [tableFitLoc[0], tableFitLoc[1], tableFitLoc[2] - 0.01,
                   np.radians(40), 0, 0]
...
# Scans the secret aruco from a defined joint position
# This position should be defined of the box actual location
...
# Returns the lid using the know returnJoints
# from pick up lid function
def returnLid(rtde_c, rtde_r, leaveLid, gripperSecretLid,
